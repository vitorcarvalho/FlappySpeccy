...
'   2. InitPipes()      — pipeCol(1) = 31    (starts at right edge)
'   3. InitPipes()      — pipeGap(1) >= 2    (lower bound of random gap)
'   4. InitPipes()      — pipeGap(1) <= 15   (upper bound of random gap)
'   5. InitPipes()      — pipeActive(2) = 0  (remaining slots inactive)
'   6. SpawnPipe(2)     — pipeActive(2) = 1  (slot becomes active)
'   7. SpawnPipe(2)     — pipeCol(2) = 31    (spawns at right edge)
...
SUB RunTestPipes()
  ...
  AssertEq("init: pipeCol(1) = 31",    31, pipeCol(1))
  ...
  AssertEq("spawn: pipeCol(2) = 31",   31,  pipeCol(2))
